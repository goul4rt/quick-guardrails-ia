...
| **Runner pago dormindo** — `sleep 180` esperando deploy propagar antes de um ping | 40 runs × ~4 min, quase tudo sono (~157 min/mês) | O sleep espera um evento que já existe: `deployment_status` dispara quando o deploy fica **pronto** (Vercel/plataformas registram deployment no GitHub). Filtrar produção em `if` de **job** — preview de PR também emite o evento, e `if` falso não sobe runner. Sem deployment registrado: cron |
