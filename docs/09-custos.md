...
Este repositório é um guideline formal de **0 a funcionando sem orçamento**: regras locais (hooks, configs versionadas), ferramentas OSS e recursos públicos/gratuitos. Artigos de DevSecOps costumam apresentar pipelines como se fossem de graça e a enforcement como se fosse automática, e nenhuma das duas coisas é verdade por padrão. Esta página fixa a regra e dá a tabela para decidir rápido.
...
4. **Free tier com rate limit nunca entra em check bloqueante**, porque cota estourada vira PR travado por motivo nenhum. Free tier serve para o que tolera falha silenciosa (ex.: análise pós-falha, [doc 08](08-seguranca-no-gate.md) camada 3).
...
| **Slack** (webhook de alerta) | Plano free aceita incoming webhook | Sem ressalva conhecida |
...
| `gitleaks-action` v2+ em organização | Exige `GITLEAKS_LICENSE` (cadastro; licença não-MIT) | gitleaks **CLI** direto no workflow, que já é o template |
...
|---|---|---|
| **Jobs paralelos multiplicam o custo fixo**: cada job paga o próprio `npm ci` E arredonda pra cima no minuto | Gate com 3 jobs: ~7 min faturáveis/run contra ~4 do mesmo gate em série (777 min/mês) | Um job só, checks em série barato→caro (o [template](../templates/.github/workflows/ci.yml) já é assim). Latência de PR não paga a conta |
...
| **Runner pago dormindo**: `sleep 180` esperando deploy propagar antes de um ping | 40 runs × ~4 min, quase tudo sono (~157 min/mês) | O sleep espera um evento que já existe: `deployment_status` dispara quando o deploy fica **pronto** (Vercel/plataformas registram deployment no GitHub). Filtrar produção em `if` de **job**, porque preview de PR também emite o evento, e `if` falso não sobe runner. Sem deployment registrado: cron |
