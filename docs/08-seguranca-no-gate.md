...
> **Origem**: artigo *Building an AI-Powered DevSecOps Guardrail Pipeline with GitHub Actions* (E. Opurum, HackerNoon, 2026) e o repo de referência dele. A arquitetura proposta (job de scan antes do build via `needs:`, alerta imediato) está certa. A implementação de referência comete erros instrutivos, analisados abaixo; extraímos o desenho e corrigimos a execução.
...
Detecção de secret é pattern-matching com baixíssimo falso positivo: regex + entropia, resultado reproduzível. O template usa o **CLI do gitleaks direto** (MIT, sem cadastro) com `fetch-depth: 0`, que varre os commits do PR e não só o estado final: secret commitado e "removido" no commit seguinte continua no histórico.
...
3. **O diff duplica abstração que já existe?** O agente reimplementa o helper que está três arquivos ao lado, e é o defeito nº 1 medido em escala (duplicação +81%). Pergunta do review: "isso já existia?"
...
Nada disso é automatizável de graça com confiança, e por isso é checklist de review, não check de CI. O que **é** automatizável já está nas outras camadas (secrets, deps, drift de imports: [doc 03](03-supply-chain.md), quinta peça).
...
Aqui o free tier do **GitHub Models** (via `GITHUB_TOKEN`, com rate limit) é aceitável, porque é exatamente o caso de uso que rate limit não quebra: roda só em falha, e se a cota estourar, o alerta sai sem a análise. Free tier **nunca** em check bloqueante ([doc 09](09-custos.md), regra 5).
...
|---|---|
| LLM bloqueante respondendo `PASS`/`FAIL` | Não-determinístico: mesmo código, veredito diferente entre runs. Gate flaky gera bypass cultural |
...
- Falso positivo do gitleaks se trata como qualquer guardrail simples ([doc 04](04-guardrails-do-agente.md)): documenta e usa `.gitleaksignore` pontual com justificativa no commit, sem desligar o scanner.
