# 12. Fronteira: guardrails de desenvolvimento versus guardrails de runtime de LLM

"Guardrail" nomeia duas categorias que não se substituem:

- **Guardrails de desenvolvimento** (este repositório): protegem o *processo* de construir software com um agente, via hooks de harness, gate de CI, supply chain, fluxo de task. O risco é o agente destruir trabalho, mergear regressão, vazar secret.
- **Guardrails de runtime de LLM**: protegem o *produto* que usa LLM em produção, validando input/output do modelo (moderação, PII, prompt injection vindo de usuário final, formato de resposta). O risco é o seu app dizer ou aceitar o que não deve.
