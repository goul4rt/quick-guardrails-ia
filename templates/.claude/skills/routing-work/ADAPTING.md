...
4. **Dono do contrato = quem define o dado.** Com 2+ repos, escolha UM como dono de schema/API e nunca relativize a regra 4 — é ela que impede migração conflitante e deploy fora de ordem.
5. **Vocabulário do tracker**: se suas skills falam papéis canônicos (`needs-triage`, `ready-for-agent`, ...), mantenha um `triage-labels.md` no repo mapeando papel → label real, em vez de editar cada skill.
...
- **A tabela de erros comuns é viva**: cada erro de roteamento que acontecer de verdade no seu time vira uma linha nela — é o changelog dos seus modos de falha.
